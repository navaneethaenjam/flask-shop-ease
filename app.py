...
def edit_product(product_id):
    ...
    # Extract size and color as lists
    updated_product_data['size'] = request.form.getlist('size[]')
    updated_product_data['color'] = request.form.getlist('color[]')

    # Remove size[] and color[] from updated_product_data
    updated_product_data.pop('size[]', None)
    updated_product_data.pop('color[]', None)
...
def add_to_cart():
    if 'username' in session:
        ...
        # Check if all required keys are present
        if productName is None or productSize is None or quantity is None or productPrice is None or productColor is None:
            return jsonify({'success': False, 'message': 'Missing form data'})

        # Check if the item already exists in the cart
        existing_item = cart_collection.find_one({'username': username, 'productName': productName, 'productSize': productSize, 'productColor': productColor})
